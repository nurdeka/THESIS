...
os.chdir("F:/THESIS/CNN/DATA/" )
os.getcwd()
dict_data = load('norm_data_ok.npz')
hs = 0

n= 14
data = dict_data[f"data_{n}"]  
#generate random numbers based on x, y to list
a = np.random.choice(np.arange(3,7475), 7472, replace=False)
b = np.random.choice(np.arange(3,2699), 2696, replace=False)
#generate tuple combination a , b
c = random.sample(set(itertools.product(a,b)), 20144512)
...
latlon1 = []
output_dir = ('F:/THESIS/CNN/DATA/ALL_7_0/')

for i, j in zip(range(0, 1908), range(1908)):
...
for i, j in zip(range(1908,7854), range(5946)):
    #print(f'coba{i}',f'dan{j}')
    #os.path.join(output_dir, savez_compressed(f"data_{i}.npz", result[i]))
    savez_compressed(os.path.join(output_dir,f"data_{hs}_{i}.npz"), result[j])
    latlon1.append(latlon[j]) 
...
output_dir = ('F:/THESIS/CNN/DATA/ALL_7_0/')
os.getcwd()
os.chdir('F:/THESIS/CNN/DATA/ALL_7_0/')
